...
struct ReadGroup {
    ...
    std::unique_ptr<std::vector<ReadTuple>>&& takeChunkPtr() { return std::move(chunk_); }
    void setChunkEmpty() { chunk_.release(); }


    //  a reference to the chunk this ReadGroup owns
    std::unique_ptr<std::vector<ReadTuple>>& chunkPtr() { return chunk_; }
...
private:
    std::unique_ptr<std::vector<ReadTuple>> chunk_{nullptr};
...
public:
    ReadParser(std::vector<std::vector<std::string>>& input_files, int numConsumers, int numParsers = 1, size_t chunkSize = 100000) : blockSize_(chunkSize), filestreams_(input_files) {
        ...
        seqContainerQueue_ = moodycamel::ConcurrentQueue<std::unique_ptr<std::vector<ReadTuple>>>(4 * numConsumers, 1 + numConsumers, 0); // blocks of empty space (ReadChunk) to fill in, the extra one is for this thread
        readQueue_ = moodycamel::ConcurrentQueue<std::unique_ptr<std::vector<ReadTuple>>>(4 * numConsumers, numParsers, 0); // blocks of filled in reads
        ...
        for (int i = 0; i < 4 * numConsumers; ++i) {
            std::vector<ReadTuple> rt(blockSize_, n_mates_);
          auto chunk = std::make_unique<std::vector<ReadTuple>>(rt);
...
private:
    ...
    moodycamel::ConcurrentQueue<std::unique_ptr<std::vector<ReadTuple>>> seqContainerQueue_, readQueue_;
    ...
    int parse_read_tuples(
        ...
        std::unique_ptr<std::vector<ReadTuple>> local;
